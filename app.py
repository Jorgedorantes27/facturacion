...
def read_excel_smart(uploaded, sheet_name=None):
    """Lee xlsx con openpyxl y xls legacy con xlrd 1.2.0 (si está disponible)."""
    name = getattr(uploaded, "name", "").lower()
    if name.endswith(".xls"):
        return pd.read_excel(uploaded, sheet_name=sheet_name, engine="xlrd")
    # default a xlsx / otros -> openpyxl
    return pd.read_excel(uploaded, sheet_name=sheet_name, engine="openpyxl")

def excel_file_smart(uploaded):
    name = getattr(uploaded, "name", "").lower()
    if name.endswith(".xls"):
        return pd.ExcelFile(uploaded, engine="xlrd")
    return pd.ExcelFile(uploaded, engine="openpyxl")
...
def build_excel(detalle: pd.DataFrame, facturas_df: pd.DataFrame, iva: float) -> bytes:
    ...
    if cols_order:
        ...
        # aquí el nombre debe ser exactamente "Fecha"
...
if sus_file and fact_file:
    try:
        sus_xls = excel_file_smart(sus_file)
        if hoja_sus in sus_xls.sheet_names:
            sus_raw = read_excel_smart(sus_file, sheet_name=hoja_sus)
        else:
            st.warning(f"No se encontró la hoja '{hoja_sus}'. Se usará: {sus_xls.sheet_names[0]}")
            sus_raw = read_excel_smart(sus_file, sheet_name=sus_xls.sheet_names[0])
        fact_raw = read_excel_smart(fact_file)
